...
# (파일, 함수A, 함수B, 사람이 코드로 확인한 진짜 차이)
# 아래는 psf/requests 기준. 다른 저장소를 검증하려면 통째로 교체한다.
CASES = [
    ("src/requests/api.py", "get", "head",
     "api.get은 allow_redirects를 지정하지 않아 request의 기본값(True)을 쓰고, "
     "api.head는 setdefault로 False를 강제한다."),
    ("src/requests/sessions.py", "get", "head",
     "Session.get은 allow_redirects=True를, Session.head는 False를 setdefault 한다."),
    ("src/requests/cookies.py", "_find", "_find_no_duplicates",
     "_find는 첫 일치 쿠키를 그대로 반환하고, "
     "_find_no_duplicates는 중복이 있으면 CookieConflictError를 던진다."),
]
...
def main() -> int:
    ...
    for i, (rel, a_name, b_name, truth) in enumerate(CASES):
        a_src = get_func(repo, rel, a_name, strip)
        b_src = get_func(repo, rel, b_name, strip)
        ...
        print(f"  [T{i + 1}] {rel.split('/')[-1]}:{a_name} <-> {b_name}")
        print(f"        진짜 차이: {truth}")
        print(f"        결과: {verdict} — {summary[:88]}\n")

        out.append(f"\n{'=' * 78}\n[T{i + 1}] {rel}  {a_name} <-> {b_name}  ->  {verdict}\n"
                   f"{'=' * 78}\n")
