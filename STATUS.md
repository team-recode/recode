...
**Updated: 2026-09-17 (수) - PHASE 13 사용자 테스트 완료 (유용률 6/9, 사용의향 1.0->5.0). 다음은 PHASE 14 배포 + 15 심사폼**
...
### PHASE 12 — 자기참조 데모 (9/16)

**Re:Code 로 Re:Code 를 분석했다.** 24절이 요구한 4가지를 `evaluation/self_reference/` 에 저장
(`outputs/` 는 gitignore 대상이라 커밋되는 위치로 복사).

| | |
|---|---|
| **분석 당시 commit SHA** | `ac8410cda3af4a5806ce01a5e1f671f9469c809a` (커밋 29개) |
| 모델 | `gemini-3.5-flash-lite` |
| 함수 | 89 → 필터 통과 77 |
| 후보 | **9쌍 전수 판정** |
| 결과 | **finding 5건 / SKIP 4 / 오류 0**, 76초, $0 (호출 9회 · 토큰 12,250) |

**finding 5건 전부 실제 코드와 대조해 사실임을 확인했다. 억지 경고 0건.**

**진짜 드리프트 2건 (발표용 강한 finding)**
1. **독스트링 정규식 범위 불일치** — `judge/embed.py:57` 은 `("""|''')` 를 처리하는데
   `scripts/make_samples.py:70` 은 `"""` 만. 뒤에 만든 것만 고치고 앞을 안 고친 전형적 드리프트.
   `'''` 독스트링 저장소에서 검증 샘플의 유사도 계산이 달라진다. **제출 후 정리 대상**
2. **테스트 파일 판별 규칙이 세 곳에서 다름** — `analyzer/test_map.py`(conftest ✗ setup ✗) /
   `judge/extract.py`(conftest ✓) / `scripts/make_samples.py`(conftest ✓ setup ✓).
   필요할 때마다 따로 만들어 뒤로 갈수록 규칙이 늘었다. **제출 후 통합 대상**

**의도한 설계였던 2건** — `make_client` 의 `SystemExit` vs `CollectError`(레이어 분리, PHASE 6 기록),
`report_page` 리다이렉트 vs `get_report` 409(화면 vs API). 질문 자체는 타당했고 답을 듣기 전엔 알 수 없는 종류였다.
→ 제품 정의("답을 정하지 않고 질문을 만든다")에 부합.

**확인 가치가 낮았던 1건** — CLI 오류 메시지 접두사 차이. 약한 finding 도 섞인다는 것을 숨기지 않는다.
...
### PHASE 13 — 사용자 테스트 완료 (9/17)

**팀 내부 2명 × A/B 교차 = 세션 4회.** 저장소는 `jd/tenacity`(finding 4) · `pyeve/cerberus`(finding 5),
규모를 맞춰(18파일 / 함수 167 vs 195) 난이도 차이로 결과가 흔들리지 않게 했다.
...
**Re:Code 질문 유용률: 6/9 (67%)** — tenacity 2/4, cerberus 4/5
**오탐 지적 3건**: cerberus [1], tenacity [1]·[2] ("굳이 질문하지 않아도 알 수 있는 내용")

**가장 강한 신호 — 사용 의향이 1.0 → 5.0 으로 갈렸다.** 두 사람이 서로 답을 모르는 상태에서
각각 A 에 1점, B 에 5점을 줬다. A 조건 1점의 이유가 둘 다 "코드가 길고 흐름 파악이 어렵다" 였다.

**부수 관찰**: 팀원 B 조건 질문 1번이 LLM finding 이 아니라 **High-change + Test evidence gap 조합**에서
나왔다("자주 바뀌는데 테스트 근거가 없는 이유가 뭔가요?"). ③④⑥ 정적 근거만으로도 질문이 생긴다는 실측.

**PHASE 9 AI baseline(68%)과 PHASE 13 사람 평가(67%)가 거의 일치.**
서로 독립적인 방법인데 같은 구간이 나왔다.

**한계 (심사 자료에 함께 기재한다)**
1. 표본 2명 · 세션 4회. 25절 요구(3~5명) 미달
2. 두 사람 다 도구 제작자 본인. **자기평가 편향 있음**
3. **언어 교란** — 대상 저장소는 영어, 리포트는 한글이다. A 조건 1점에 영어 장벽이 섞여 있어
   **A/B 격차가 실제 도구 효과보다 부풀려져 있을 수 있다**
4. A 조건은 파일 수준(`validator.py`)까지만, B 조건은 `파일:줄` 까지 제시됨
5. **통계적 유의성을 주장하지 않는다** (25절)

> 표기: **"팀 내부 2명 교차 테스트(세션 4회), 자기평가 편향 및 언어 교란 있음"**
> 원자료: `evaluation/user_test/응답시트.md`

## In Progress

- PHASE 10·11·12 완료. **브라우저에서 URL 을 넣으면 보고서까지 나오고, 자기참조 데모 자료도 확보됐다.**
- 다음: **PHASE 14 배포(26절)** 는 코드 작업, **PHASE 13 사용자 테스트(25절)** 는 사용자 오프라인 몫(개발자 3~5명 섭외)
...
| TSX | ✅ radix-ui 1,299함수 | ✅ 10쌍 / **4건** | 41.5% | 9/18 완료 |
| C | ✅ cJSON 1,041함수 | ✅ 8쌍 / 2건 | - | 매크로 래퍼(`CJSON_PUBLIC(...)`) 통과 |
| C++ | ✅ spdlog 2,208함수 | ✅ 10쌍 / 1건 | - | 9/18 완료 |
...
**스토리북 · 예제 · 목업 파일 제외 (9/18)**

radix-ui/primitives 를 돌려보니 후보 200개 중 99개가 `.stories.tsx` 였다. 스토리는 "일부러 다르게 만든" 예제라 서로 비교하면 `PopoverNonModal` 과 `PopoverFullyModal` 의 차이를 묻는 무의미한 질문이 나온다. `judge/languages.TEST_DIRS` 에 `stories` · `storybook` · `__mocks__` · `fixtures` 등을 넣고 `.stories.*` · `.story.*` 파일명 규칙을 추가했다. 적용 후 후보 200개가 전부 production 코드가 됐다. mhctools Python 회귀는 다시 0건.

**알고 있는 한계**

- **JS/TS 이름 해석률 31~41%.** 나머지는 `$.each(fields, function (i, f) {...})` 같은 익명 콜백이다. 이름이 없으면 "이 함수를 왜 이렇게 했는지" 를 가리킬 수 없어 버린다. 의도한 동작이지 버그가 아니다
- ~~TSX · C++ LLM 판정 미완~~ → **9/18 완료.** 지원 언어 8개(HTML 제외) 전부 실저장소 + LLM 판정까지 확인했고 API 오류는 spdlog 0건 / radix 1건(429, 자동 복구)
...
"외부 API 를 쓴다" 만 보면 저장소 전체를 보냈다고 오해한다. 실제 줄 수를 재서 보여준다.

- `judge/llm.sent_code_summary()` 가 분석할 때마다 실측해 `metrics.json` 의 `privacy` 에 넣는다
- 리포트 화면(언어 구성 아래)과 `HANDOFF.md` 가 같은 값을 쓴다
- 실측: gson 0.59%, serilog 0.40%, cJSON 0.72%, axios 1.64%, cachecontrol 7.78%, mhctools 13.09%
  (작은 저장소일수록 비율이 높다. 분모가 작아서다)
- gson 기준 유사도 비교 190,653번이 전부 로컬, 그중 15쌍만 외부로 나갔다

**사이트에는 사실만, 권고는 문서에.** "로컬 LLM 교체 권고" 는 README `## Data handling` 과
심사 폼·발표에만 넣는다. 자기 제품 페이지에 "보안 때문에 기업은 못 쓴다" 를 써두면
심사위원이 그것부터 읽는다. 같은 사실을 설계 선택으로 제시하는 편이 강하다.

⚠ **현재 코드는 Gemini SDK 를 직접 호출한다.** 교체 지점이 `judge/llm.py` 한 파일로
모여 있을 뿐, 로컬 LLM 연동은 구현하지 않았다. "지금 되나요?" 를 물으면
"설계상 한 파일만 바꾸면 되고, 아직 구현은 안 했다" 가 정직한 답이다.

### 모델 선택 재검증 — 임베딩 교체 불필요, 병목은 LLM 하루 한도 (9/18)

"더 좋은 모델이 있는가", "유료로 바꾸면 빨라지는가" 를 실측으로 확인했다.

**① 시간의 8할은 외부 API 단계다 (코드량 0.59% 와 혼동하지 말 것)**

| 저장소 | 총 시간 | LLM 단계 | 비중 |
|---|---|---|---|
| axios/axios | 126.7초 | 122.4초 | 96.6% |
| google/gson | 120.3초 | 115.8초 | 96.3% |
| pyscript | 180.1초 | 175.0초 | 97.2% |
| **14개 저장소 합계** | **2,724초** | **2,164초** | **79.4%** |

외부로 나가는 **코드량** 은 0.59% 지만 **시간** 은 79.4% 다. 두 수치는 다른 것을 재고 있다.
그리고 그 LLM 시간의 대부분은 추론이 아니라 대기다 — 무료 티어 분당 제한 때문에
요청 사이를 7초 재운다. axios 는 122초 중 98초가 대기였고 실제 호출은 1.6초/건이었다.

**② 유료 전환 시 예상 (실측 기반)**: 간격 제거만으로 2,711초 → 1,703초, 4병렬까지 하면 840초.
**전체 3.2배, 작은 저장소는 최대 12배**. 모델이 빨라지는 게 아니라 대기가 사라지는 것이다.

**③ 심사위원이 저장소를 주는 시나리오에서는 속도보다 하루 한도가 위험하다**

- 무료 총량 = 모델 5개 x 하루 20회 = **하루 100회**
- 중형 저장소 하나의 후보가 **100쌍** (django · gson · spdlog · serilog · radix 모두 100)
- 즉 **저장소 하나를 전부 판정하면 그날 할당량이 끝난다.** 소요 15분
- 현재 웹은 `WEB_MAX_PAIRS=20` 으로 잘라 하루 5회까지 가능 (회당 약 3분)

**④ 임베딩 모델 4종 비교 — 바꿀 이유 없음**

사람이 라벨링한 31건(useful 21 / not_useful 8 / ambiguous 2)을 정답지로 썼다.
후보 생성까지만 보므로 LLM 호출 0, 비용 0.

| 모델 | useful 재현 | ≤20위 | 순위 중앙값 | 오탐 후보 | 인코딩 |
|---|---|---|---|---|---|
| **all-MiniLM-L6-v2 (현재, 87MB)** | 21/21 | 20 | 8 | 8/8 | **88초** |
| all-mpnet-base-v2 (418MB) | 17/21 | 15 | 9 | 4/8 | 764초 |
| BAAI/bge-small-en-v1.5 (128MB) | 15/21 | 12 | **7** | **2/8** | 253초 |
| st-codesearch-distilroberta (코드 특화) | 14/21 | 9 | 16 | 6/8 | 210초 |

⚠ **현재 모델의 21/21 은 실력이 아니다.** 정답지가 이 모델이 뽑은 후보를 라벨링한 것이라
구조상 100% 가 보장된다. 이 표에서 읽을 수 있는 것은 **"대안들이 현재 모델이 찾은 유용한 쌍조차
4~7건 놓친다"** 뿐이다. 대안이 *대신* 찾아낸 좋은 쌍은 라벨이 없어 보이지 않는다.
그것까지 보려면 대안 모델의 새 후보를 LLM 으로 판정해야 하고 할당량을 쓴다.

**결론: 임베딩은 그대로 둔다.** 속도가 3~9배 나빠지는데 품질 이득의 근거가 없다.
다만 `bge-small` 이 오탐을 8/8 → 2/8 로 줄이고 순위 중앙값도 가장 좋았다.
정밀도만 따로 필요해지면 재검토 대상.

**⑤ Claude · Codex 는 임베딩 모델이 아니다**

둘 다 텍스트 생성 LLM 이라 ②번(유사도 계산) 자리에 못 들어간다.
Anthropic 은 임베딩 모델을 만들지 않고 Voyage AI 를 권한다
(https://docs.anthropic.com/en/docs/build-with-claude/embeddings).
Codex 는 OpenAI 의 옛 코드 완성 모델로 지금은 없어졌다.

**API 임베딩으로 갈아타면 프라이버시 설계가 무너진다.** 지금은 전체 함수를 로컬에서
벡터로 만들고 좁혀진 후보만 외부로 보내 0.59% 를 유지한다. 임베딩을 API 로 바꾸면
**모든 함수 소스가 외부로 나간다.** 0.59% 가 100% 가 된다. 속도·품질 이전에
이 구조를 깨는 선택이라 검토 대상에서 제외한다.
